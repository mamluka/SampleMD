...
!*******************************************************************************
      ...
      if (new) then
          ...
          do i=1,lr-ip-lss+1
              s%chars(i+ip+lss-1) = s%chars(if+i)
          enddo
          do i=lss,1,-1
              s%chars(i+ip-1) = ss%chars(i)
          enddo
      endif
      s%len = lr
...
!*******************************************************************************
      ...
      if (new) then
          ...
          do i=1,lr-ip-lc+1
              s%chars(i+ip+lc-1) = s%chars(if+i)
          enddo
          do i=lc,1,-1
              s%chars(i+ip-1) = c(i:i)
          enddo
      endif
...
#ifdef TR15581
      if (allocated(s%chars)) deallocate(s%chars)
      allocate(s%chars(max(lr,string_size(s))))
      s%chars(1:lr) = rs
      deallocate(rs)
#else
...
!*******************************************************************************
      ...
      integer                      :: r(size(a))
type(string) :: b(size(a))


      call lrank(a,r)

!*** Some compilers make mistakes with the following line
!     a = a(r)
b = a(r)
a = b
call unstring(b)
...
!*******************************************************************************
   ...
   10 continue
      p = (i + j)/2
      if (lgt(a(i),a(p))) call swap_strings(a(p),a(i))
      if (llt(a(j),a(p))) then
          call swap_strings(a(p),a(j))
          if (lgt(a(i),a(p))) call swap_strings(a(p),a(i))
      endif
...
!*******************************************************************************
   ...
   10 continue
      p = (i + j)/2
      if (a(i) > a(p)) call swap_strings(a(p),a(i))
      if (a(j) < a(p)) then
          call swap_strings(a(p),a(j))
          if (a(i) > a(p)) call swap_strings(a(p),a(i))
      endif
...
!*******************************************************************************
   ...
   10 continue
      p = (j+i)/2
      if (lgt(a(r(i)),a(r(p)))) then
          rt   = r(p)
          r(p) = r(i)
          r(i) = rt
      endif
      if (llt(a(r(j)),a(r(p)))) then
          rt   = r(p)
          r(p) = r(j)
          r(j) = rt
          if (llt(a(r(i)),a(r(p)))) then
              rt   = r(p)
              r(p) = r(i)
              r(i) = rt
          endif
...
!*******************************************************************************
#ifdef XDR
      subroutine xdr_string(id,s,status)

      use xdr

      implicit none
      type(xdr_id), intent(in)        :: id
      type(string), intent(inout)     :: s
      integer, intent(out), optional  :: status
      integer                         :: l


      select case(fxdr_mode(id))
      case(XDR_MODE_CLOSED)
          if (present(status)) status = -100
      case(XDR_MODE_READ)
          call fxdr(id,l)
          if (l > s%size) call resize_string(s,l)
          call fxdr(id,s%chars)
          s%len = l
      case(XDR_MODE_APPEND,XDR_MODE_WRITE)
          call fxdr(id,s%len)
          call fxdr(id,s%chars(1:s%len))
      end select

      end subroutine xdr_string
#endif
!*******************************************************************************
